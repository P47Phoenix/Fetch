...
from fixtures import REDIR_BODY, REDIR_FINAL, HOSTILE_SIZE
fixtures_hostile_size = lambda: HOSTILE_SIZE
MIB = 1024 * 1024
CAP = 5 * MIB
...
SCENARIOS = {
    ...
    # refused with converter_limit (before the fix: converted, ~114 MB RSS each). `hostile-attrvalue3`: one 1.9 MiB attribute value,
    ...
    "hostile-attrs3":  dict(kind="peak", gate="none", route="/hostile-attrs.html", args={"max_length": 5 * MIB}, expect="converter_limit",
                            concurrency=3, recorded_only=True, min_bytes=lambda m: 3 * 4096, implemented=True,
