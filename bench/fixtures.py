...
HOSTILE_SIZE = 2 * MIB - 4096  # fix-pass 1: hostile pages sit just under the 2 MiB tokenizer limit, so the limit alone never trips on them
...
def hostile_body(kind):
    ...
    ONE attribute value of ~1.9 MiB (legitimate shape: converts, bounded by the tokenizer limit)."""
    ...
    if kind == "attrvalue":
        ...
        n = HOSTILE_SIZE - len(pre) - len(post) - len(head) - len(tail)
