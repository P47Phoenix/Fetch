...
Status: Proposed (allocator part: Accepted on x86 evidence; libc choice pending native ARM data)
Date: 2026-09-19
...
- Candidate tuning to test on the native runner, not assumed: glibc `mallopt(M_ARENA_MAX=1)` and `M_TRIM_THRESHOLD`/`M_MMAP_THRESHOLD` (few threads: `current_thread` runtime plus tokio's blocking thread for stdin), and a release `opt-level` 3 vs `s`.

Decision criteria (evaluated on the author's native aarch64 runner, each figure median of 10 fresh processes, both binaries, same fixtures, page size recorded):
