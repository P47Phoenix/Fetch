...
## Amendment 2026-09-20 (A-3b: how the dial route is realised; closes Sprint 1 architect items N1 and N2)
1. **Per-hop client with a pinned resolver.** Instead of one long-lived client with a global `dns_resolver`, `src/fetch/mod.rs` builds a client per hop whose `dns_resolver` is `fetch::dns::Pinned`: it returns exactly `Validated.addrs` for exactly the validated host and refuses every other name. The single lookup is made by our own `SystemResolver` (tokio `lookup_host`, at most 16 answers kept in resolver order, architect N5) inside `validate_target`, before the client exists; the client never resolves. No fallback to reqwest's own resolver exists in the build (the merge-gate test scans the source for any second route). IP-literal hosts never reach a resolver; they were judged by `check_url`.
2. **URL cross-check at runtime (N2a).** Before every request the URL is parsed with the `url` crate (reached through reqwest) and its scheme, port and host must equal what `check_url` validated (an IP host must be the same address, a name the same lower-case name without trailing dot, no userinfo); any mismatch is refused as `blocked_target`. In addition, `src/ssrf/differential.rs` compares `check_url` against `url::Url::parse` over an encodings corpus, every spelling of a set of IPv4 addresses and a 150,000-input deterministic fuzz corpus (`url` as a dev-dependency only), with properties (A) nothing the reference sees as blocked passes, (B) whatever passes is read identically, (C) userinfo never passes; a positive control proves the checker can fail. No disagreement was found. Result: N2a closed.
3. **N2b closed:** the named test `a3b_merge_gate` proves the constructor takes a `Policy` with no `Default`, that a client built by the hop path dials the validated host and refuses other names with zero connections, that a blocked answer causes zero connections, and that no other socket or resolution route exists in the source.
4. **N1 closed:** the `url` crate is no longer mandated (Amendment 2026-09-19 above; architecture 9.1, 14 R13 and the amendment list carry the count and pin removal). Direct dependency count after A-3b is 8 of 15 (rmcp, tokio, serde, schemars, reqwest, rustls, webpki-roots, flate2); `url` is a dev-dependency only.
5. Redirect hops: fixed bound of 5 (`too_many_redirects` on the sixth); `Location` is resolved against the current URL with `Url::join` and the absolute result is re-run through `revalidate_hop` semantics (`Origin::Redirect`). The configurable limit and its extra tests remain B-3.
