...
Errors (see Amendment 2026-09-19 below: parameter rejections are `isError` results, not JSON-RPC errors): runtime failures are `isError: true` results with `error[<code>]: <message>` (architecture.md 6). No structured output schema in v1 (plain text content only), keeping client compatibility and memory low.
...
## Amendment 2026-09-19 (A-3a, Accepted ADRs may be amended with a dated note)
Decision text above says parameter rejections are "JSON-RPC invalid params". rmcp 3.4.0 reports schema-deserialisation failures (missing or wrong-typed `url`, negative or non-integer numbers, wrong-typed `raw`) as an `isError: true` tool result whose text names the field, which matches the current MCP guidance that input-validation errors be tool execution errors so the model can correct itself. Getting protocol-level errors for these would need a custom `call_tool` handler that bypasses rmcp's `Parameters` extraction (a hack that also loses the generated schema wiring). Amended rule, uniform for every rejection of the initial `url` and of the other parameters: an `isError: true` result with text `error[invalid_argument]: <field>: <message>` (or `error[blocked_target]: ...` for a blocked IP literal or `localhost`), naming the field; never a JSON-RPC error. Redirect `Location` handling is unchanged (`blocked_target`). Architecture 6.1 row `invalid_argument` and 6.2 read accordingly. Tests (`tests/stdio.rs`) assert `isError` and fail on a protocol error.

## Amendment 2026-09-20 (OQ-5 decided: no untrusted-content label; dated note, decision text above left as written)
Decision item 7 keeps room for an "optional untrusted-content label (OQ-5)". The owner (Michael) decided OQ-5 on 2026-09-20: **NO label.** Fetched content is returned as-is. The result envelope therefore carries no label, wrapper, prefix, notice or extra content block for untrusted content; the only things outside the offset counter remain the A-9 `Final URL` header and the footer of item 3. Consequences: story B-6 is closed as won't-do (EPICS B-6, PRD OQ-5); the A-3b/A-4 result is a single text content item; prompt-injection risk (PRD risk 5) is mitigated by documentation only. A-3b already returns the text unlabelled (`src/server.rs`, asserted end to end in `tests/stdio.rs` under `bench-loopback`).

## Amendment 2026-09-21 (A-5, Sprint 5; dated note, decision text above left as written)
Decision item 4 has the result state the total length. As built, `[Total length: N characters.]` is stated only on a continuation page (`start_index` > 0) and in the beyond-end message; a first page that fits carries no footer, so a small page is returned as fetched (consistent with OQ-5, no label). The total is known only if the read reached the end (early stop otherwise). Pending owner acknowledgement.
