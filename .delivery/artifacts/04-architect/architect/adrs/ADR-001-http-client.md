...
- Dependency note (fix-pass 1, 2026-09-20): reqwest's `rustls-no-provider` feature unavoidably links `rustls-platform-verifier` and `openssl-probe` (Rust crates; they do not link OpenSSL, and `cargo deny` still bans openssl, native-tls and aws-lc). They are compiled in but unused at runtime because we supply our own `tls_backend_preconfigured` config. Trimming them is not possible without dropping reqwest's feature set; D-1 should account for their size in the binary, not try to remove them.
