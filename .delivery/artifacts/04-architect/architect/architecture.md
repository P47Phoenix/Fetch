...
| 005 | System allocator; libc choice (gnu vs musl) pending ARM data, now measured inside the image; standalone-binary parts SUPERSEDED by 007 | Proposed, amended by 007 |
| 007 | GitHub-hosted arm64 runners for the ARM gates; tested container image on GHCR is the release artifact (digest-gated); standalone binaries dropped | Accepted (user, 2026-09-19) |
...
### 9.2 Runner topology and trust (resolves DevOps B2; REVISED by ADR-007: hosted runners only, no self-hosted runner)

All jobs run on GitHub-hosted runners: `ubuntu-24.04` (x86_64) for lint, tests, deny, audit, feature guards and the AMD64 image measurement, and `ubuntu-24.04-arm` (native arm64, public repo) for the ARM image measurement. Both platforms are hard-gated natively; the hosted arm64 runner is a cloud VM, not the author's Pi 5 cluster (see 9.3 caveats).

| Trigger | amd64 hosted jobs (`ubuntu-24.04`) | arm64 hosted jobs (`ubuntu-24.04-arm`) |
|---|---|---|
| Pull request from same repo | fmt, clippy, unit/integration tests, `cargo deny`, `cargo audit`, feature-guard checks, native amd64 image build (load, no push), handshake and FR-15 checks, reduced memory smoke | native aarch64 build and test suite (D-3), image build (load, no push), image handshake and FR-15 checks, memory smoke (reduced) once E-2 exists; no manual approval label needed |
| Pull request from a fork | same jobs, no secrets, read-only token | same jobs, no secrets, read-only token; images are never pushed. The workflow contains no `pull_request_target` trigger |
| Push to `main` | as above plus full memory benchmark on the amd64 image and baseline store | full memory benchmark on the arm64 image (E-2..E-4) and baseline store; no registry push |
| Tag (release) | build amd64 image by digest; run the full amd64 gates on the manifest digest | build arm64 image by digest; run the full arm64 gates on the manifest digest; a merge job builds the manifest list; a publish job promotes the SAME manifest digest only if BOTH gates pass (9.3) |
| Nightly schedule | `cargo audit`; full matrix benchmark on amd64 | full matrix benchmark (gnu and musl candidate images until ADR-005 decides) |
| `workflow_dispatch` | any | any (maintainer); the only re-run path for the gates |

Rules kept from the previous design (still required): least-privilege `GITHUB_TOKEN` (workflow default `permissions: contents: read`; `packages: write`, `id-token: write` and `attestations: write` granted per job only on the tag publish path), NO `pull_request_target` anywhere, every third-party action pinned by full commit SHA, no secrets exposed to fork PRs. Removed as self-hosted-only: ephemeral runner registration, LAN/TrueNAS/Home Assistant isolation rules (a hosted VM has no route to the user's network), label-gated PR runs on self-hosted, dedicated-runner assumption. The hosted VM is not dedicated: record load average before and after and the CPU model in every report; the load-suspect rule stays.

Availability (replaces "offline runner"): risk is hosted-runner availability, arm64 queueing and Actions quotas. If either platform's jobs cannot run, that gate cannot be evaluated and the release is BLOCKED. Skipped is failure: branch protection marks the arm64 test and memory-gate jobs as REQUIRED status checks on `main` and the release workflow `needs:` them. QEMU (`setup-qemu-action`, binfmt) must never be used in a measurement job and must not substitute for the memory gate. The only accepted fallback is a `workflow_dispatch` re-run against the same digest; a local or Pi run is informational and is never accepted as release evidence.

### 9.3 Container image build and release (ADR-007)

- Base: minimal and non-root, pinned by digest (`FROM gcr.io/distroless/static-debian12:nonroot@sha256:...` for musl static, or `distroless/cc-debian12:nonroot@sha256:...` for gnu; `scratch` with a numeric `USER` is acceptable for musl). No shell, no package manager; roots embedded (ADR-001). Base digests are refreshed by Dependabot (docker ecosystem) as their own PR, which must re-run both platform gates.
- Platforms: `linux/amd64` and `linux/arm64`, one manifest list built from per-platform images; each platform image is built and measured on its native hosted runner. Deferred, considered and not rejected: Windows containers, arm/v7, 386, riscv64, ppc64le, s390x. Policy for any future platform we can build but cannot natively memory-verify: publish only with the label "memory targets not verified on this platform" in image labels/description and docs; QEMU never counts as a gate; amd64 and arm64 stay hard-gated.
- Build: multi-stage Dockerfile, `--locked`, same pinned toolchain and D-7 profile; the binary is built for the chosen libc and copied into the final stage; build args carry the commit and Cargo.lock hash so `--version` matches. A `bench` target (same Dockerfile, same base digest, `bench-loopback` feature) is a second image that is never pushed or tagged; the shipped image and the bench image differ only by that binary.
- Digest gate (multi-arch): (1) per-platform jobs on the matching native runner build and push the image by digest, untagged, private (D_amd64, D_arm64); (2) a merge job creates the manifest list M from those digests, untagged and private; (3) per-platform test jobs pull `@sha256:M` on their native runner, assert the resolved per-platform digest equals D for that platform, and run the handshake in a clean container, `--version`, non-root check (`docker inspect`), FR-15, the D-7 guard and marker grep on the binary extracted from that digest, and the memory gates (idle on the shipped image, peak on that platform's never-pushed bench image); (4) only if BOTH platform gates pass, a publish job adds version (and `latest` per OQ-7) tags to M without rebuilding, and attaches provenance and SBOM attestations (recommended). The measured per-platform digest is the published one and M is the tagged one. Release notes record M and both D. Tagging before both gates pass on these digests is forbidden.
- Attestations: buildx provenance and SBOM plus `actions/attest-build-provenance` for the digest are recommended (optional for MVP; do not block on them). Any attestation manifests added to the index must be part of the M that is tested and tagged.
- Visibility: the first push creates a private package. Making it public is a manual, deliberate act that is a distribution decision; OQ-7 must be answered before the first image is published (it is already due before Sprint 9; ordering confirmed: OQ-7 precedes the first D-2 tag run, and the pipeline may be built and exercised with private candidates earlier).
- Measuring inside a container: the gate reads VmRSS and VmHWM from `/proc/<pid>/status` of the server process (PID 1 of `docker run -i --rm`, host pid from `docker inspect`), never `docker stats` or cgroup `memory.current`. Container limits do not change these counters below the limit; the gate runs without a limit and a `--memory=64m` confirmation run must not be OOM-killed (recorded only). Runtime daemon and shim processes are excluded from the figure and this is stated in every report. Preflight adds `uname -m` inside the container (must equal the platform under test) and refuses any run where QEMU or a foreign-architecture binfmt handler is present.
- Libc and allocator in the image: system allocator; exactly one libc flavour is published, chosen by ADR-005 criteria on data measured inside the image. Recommendation (no user decision needed unless the data is a tie or both fail): musl static on distroless/static if parity holds (the glibc floor is moot inside an image), else gnu on distroless/cc. Both candidate images are built and measured until the decision. musl DNS in containers goes through the runtime resolver, so `.local`/split-DNS is tested from inside the container and documented.
- Hosted-runner caveats for reports (a per-platform host-facts table, amd64 and arm64): record OS, kernel, CPU model, RAM, `getconf PAGESIZE` (expected 4 KiB on both; Raspberry Pi 5 kernels default to 16 KiB, which can inflate RSS; a 4K pass does not prove a Pi pass), THP, runtime and version. Same-page-size comparisons only.
...
- Release integrity (REVISED by ADR-007): the artifact is an image identified by digest; provenance and SBOM attestations recommended (see 9.3); no per-binary SHA256 checksums or macOS codesign. Distribution channel is OQ-7 and is a precondition for publishing a public image; `cargo install` from source remains possible only if OQ-7 decides it (the RSS claim covers the published image only). UA string embeds a repo URL that must be a build-time constant, not a TBD in code.
- Roots: see ADR-001 (webpki-roots embedded). Unmeasured on ARM.
- Feature-guard CI (R12): D-7 (Sprint 0, skeleton with a self-test positive control, `-p <crate>` release build) and D-2 (the binary inside every release image, extracted from the tested digest) assert that `test-support`, `bench-loopback` and the bench-only fixture-CA feature (section 11 item 7) are all absent: `cargo tree -e features` on the release build PLUS a marker-string grep on the actual artifact (the `bench-loopback` build embeds a fixed marker string, so the grep is reliable). Cargo features are additive, so the guard is also a required CI check from Sprint 1. E-8 unit tests run WITH the feature in hosted CI.
...
5. Record per report: host CPU, kernel, `getconf PAGESIZE`, RAM, cgroup limits / container flag, load average before and after, libc (gnu/musl), allocator, binary sha, commit, cargo profile, toolchain version, fixture sha256s. macOS uses `/usr/bin/time -l` (not comparable to VmHWM; reported separately).
...
- Runner state: hosted runners are cloud VMs, so the CPU governor is not controllable and is not a gating rule (ADR-007); no concurrent load (load average recorded before and after; > 0.5 above idle baseline marks the run suspect and it is repeated once), fixture server and harness on the same host over loopback.
...
- Native-ARM preflight (refuse to emit gating numbers otherwise; also run inside the container): `uname -m` = aarch64; `file <binary>` shows ARM aarch64; no qemu binfmt handler registered for aarch64 in `/proc/sys/fs/binfmt_misc`; `getconf PAGESIZE` recorded and only same-page-size runs compared (16K/64K hosts inflate RSS).
...
10. PRD OQ-9 wording: SUPERSEDED by ADR-007 (hosted arm64 runner; self-hosted trust model removed). OQ-3/4/5/7 remain open; nothing here decides them.
