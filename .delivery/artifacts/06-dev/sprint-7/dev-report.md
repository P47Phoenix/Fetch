...
Not done (blocked): the live 10-URL smoke run and a published "release"/gap report in `docs/`. No URL list was invented. Owner must supply: a list of exactly 10 public https URLs (no cookies/auth), covering TLS, at least one redirect, one JSON, one plain-text page. Plan impact: the plan defines no fallback; E-5 cannot close, Goals 2 and 3 have no live evidence, no release decision can be published, MVP tagging waits. Sprint 7 is 8 pts, so it lands at 6 delivered plus E-5 carried; Sprint 8 has 2 pts of room and is the only sprint that fits E-5 whole; Sprints 9 and 10 have 1 pt each; Sprint 12 is not claimed. The report can be generated from CI artifacts the moment the list exists (`smoke.py` then `e5_report.py`); CI wiring of the smoke step was deliberately not added because it needs the list committed.
...
## Fix pass 1 (code review REQUEST_CHANGES, QA/docs NOT_DONE)
Blocking, fixed: (1) `e5_report.py` now needs at least one success in each of tls, redirect, json, text and at least one followed redirect, else exit 2 (a 0/10 smoke never releases); `smoke.py` rejects IP-literal hosts in real runs; truncated DONE summaries exit 2. (2) A malformed URL in the list is the INVALID path (exit 2, summary and `--out` written). (3) The overhead line must be 1048576 bytes on aarch64 or x86_64 and never under qemu. Selftests added for each.
Non-blocking, fixed: a3b-merge-gate filter and grep allowlist now include the b2_/b3_ tests (WORKFLOW EDIT in ci.yml; required-check set unchanged); `--child-env` without `=` is a clean error; `#` inside a URL is a fragment (only whole-line comments); b2 request-URL test asserts `blocked_target` exactly; the loopback hop test is renamed `b2_revalidate_hop_refuses_every_loopback_spelling` (it tests `revalidate_hop`, not the loop; the loop is covered by the b3_ encoded redirect test on non-loopback targets, since the default policy cannot reach a loopback fixture); room claim corrected; EPICS.md "E-5 landed" fixed; README Sprint 7 status added.
Follow-ups (not fixed): the hung real-`getaddrinfo` blocking-thread limit is untracked engineering debt; smoke.py category labels are owner-supplied and unverified (a URL tagged json is not checked to be JSON); `ok` counts any non-empty result, including an HTML error page returned with a 2xx; tls and redirect are inferred from scheme and the `URL:` header rather than verified.
