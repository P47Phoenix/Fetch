...
have=$(cargo zigbuild --help >/dev/null 2>&1 && echo yes || echo no)
[[ $have == yes ]] || { echo "build-candidates: cargo-zigbuild not installed (cargo install --locked cargo-zigbuild --version $ZIGBUILD_VERSION)" >&2; exit 2; }
