...
### Two variables' *defaults* are still open product policy (OQ-3, OQ-4)

Both variables are real, working mechanisms (not placeholders), but the product owner has **not yet decided**
whether either should be turned on **by default**; both default to their pre-existing, conservative behavior
(robots.txt never fetched, private-host allowlist empty) until a decision lands.

- **`FETCH_ROBOTS_TXT` (OQ-3, default policy -- open; mechanism itself is implemented, B-4).**
  Setting `FETCH_ROBOTS_TXT=enforce` makes every fetch first fetch and check the target origin's
  `robots.txt`, refusing a disallowed path with `error[robots_disallowed]`. Details:
