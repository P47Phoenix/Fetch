...
**Red checks on PR #9 that are not G4b.** The two arm-bench jobs `bench (gnu)` and `bench (musl)` (workflow `arm-bench.yml`) are red. They measure the Sprint 0 A-1 spike, which has no `start_index`/`max_length`; the peak scenarios are window requests, so they resolve INVALID and the job's summary step then stops on a missing `metric` field. These jobs are advisory and not required checks. The fix (dropping the spike's peak measurement) is a decision pending with the owner and is not made in this PR.
