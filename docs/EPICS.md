...
- Given G0 uses A-1 measurements taken before this protocol existed, when the spike ends, then it either re-runs the A-1 binary under this protocol (median of 10 valid runs, MB definition above, release profile as pinned in D-7 or the deviation noted) or records the deviation.
...
- Given a run, when results are produced, then each figure is the median of 10 valid runs with min and max shown, and the harness rejects (non-zero exit) a result set with fewer than 10 valid runs. The unit for MB is the E-1 definition, used for targets, fixtures and caps alike.
...
- Given the 5 MB HTML fixture, when `fetch` runs on the `bench-loopback` build, then peak RSS is at or below 40 MB; the shipped-binary cross-check (idle delta versus bench build, binary size delta, and one manual 5 MB fetch of a public host on the shipped binary) is recorded and checked against the E-8 bounds.
- Given Sprint 4 precedes A-5 (window and early stop) and A-6 (`raw=true`), when G4a is evaluated, then it covers idle plus only scenarios that need A-3b and A-4: 5 MB HTML fully read and converted, the same page gzipped, late-landmark holdback-full HTML, 50 MB with Content-Length, and 50 MB chunked read without a window beyond the cap; 10 concurrent recorded, not gating. Nothing that needs the window, early stop or `raw` is in G4a. Those scenarios (window at start, window at end, chunked window inside the cap, window beyond the cap, `raw=true`) are gated as G4b at the end of Sprint 5 against the same targets, owned by A-5 and A-6 (non-blocking for E-4's own Done). The complete memory gate is therefore closed at the end of Sprint 5, not Sprint 4.
...
- Given the 50 MB fixture served chunked (no `Content-Length`) and a full read without a window, when `fetch` runs, then reading stops at the cap, the call returns a `too_large` error and peak RSS is within 10% of the 5 MB-page peak. (The windowed variants, chunked window inside the cap succeeding and window beyond the cap, need A-5 and are G4b scenarios.)
...
- Given E-3, E-4 and the G4b results, when the report is generated, then it states the release profile measured (pinned in D-7, finalised and re-measured in D-1, Sprint 8; the D-1 re-measure is the figure that governs the MVP tag) and it lists idle RSS, peak RSS, and 50 MB boundedness against their absolute targets.
...
- Given both binaries built from one commit, when compared, then the only source difference is the cfg'd policy constructor; the size delta and idle RSS delta are recorded, and the bounds are: idle delta at most 0.5 MB, binary size delta recorded (no bound, must be explained), and one manual 5 MB fetch of a public host on the shipped binary with peak within 10% of the bench-build peak and at or below 40 MB. Exceeding any bound fails G4a until explained and re-measured, so a diverging bench build cannot pass. Both binaries must report the same commit and Cargo.lock hash in `--version`.
...
- Given a URL returning a 10 MB chunked body (no `Content-Length`) when `fetch` reads it in full, then reading stops at the limit and the call returns a "too large" error. (Window-based behaviour, a chunked body whose requested window completes under the limit succeeding, arrives with A-5 and is tested there.)
...
- Given the window and early-stop path (`Window` sink) is in place, when G4b is prepared, then the harness scenarios that need it (window at start, window at end G1, chunked window inside the 5 MB cap succeeds, window beyond the cap returns `too_large`) run on native aarch64 on the D-7 release profile (gnu and musl, median of 10 valid runs, peak on the bench build), each at or below 40 MB and the 50 MB chunked cases within 10% of the 5 MB peak. A-5 owns these runs and keeps the harness scenarios for them working (in scope, no extra points).
...
- Given no variables, when the server starts, then it uses defaults: 15 s, 5 MB, `max_length` cap 100,000, concurrency 3, block private, and the robots toggle parsed with a placeholder default only (C-1 does not decide OQ-3; the default is confirmed when OQ-3 is decided, due before Sprint 10 starts).
...
- Given `deny.toml` (committed in D-7), when the release pipeline runs, then it still has `advisories`, `bans` (deny `openssl`, `openssl-sys`, `native-tls`, `aws-lc-sys`, `aws-lc-rs`), `sources` (crates.io only) and `licenses` (permissive allow-list) sections, and CI asserts the release build has none of `test-support`, `bench-loopback` and the bench fixture-CA feature.
