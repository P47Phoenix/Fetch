...
- **A-7 status (Sprint 6).** Implemented: every failure is an `isError` result `error[<code>]: <message>`; argument failures (missing, wrong type, out of range) are uniformly `error[invalid_argument]: <field>: <why>` (the rmcp `failed to deserialize parameters:` prefix is gone; the advertised schema is unchanged); HTTP 4xx and 5xx messages name the class and reason; an `internal` error shows a generic message and logs its detail to stderr. Tests assert flag and text for each cause (`src/server.rs`, `src/fetch/tests.rs` `a7_each_cause_is_flagged_and_names_itself`, `tests/stdio.rs`). Deviation: the third AC ("unexpected internal error ... keeps running") holds for `Err` paths only; a panic aborts the process because of `panic = "abort"` (memory budget, D-7), so it cannot return a result. Needs owner acknowledgement.
