...
- The 95% conversion success and 50% median token reduction checks run in A-4 (Sprint 4); this story only supplies the set (E-7 was planned for Sprint 2 but has slipped: it needs the owner's URL lists, so the A-4 checks run when E-7 lands and A-4 is not Done until then).
