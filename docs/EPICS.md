...
- Given the E-8 acceptance items that need the harness (moved here from E-8 in A-3b fix-pass 1): the handshake reports the bench marker, the idle RSS delta (bound 0.5 MiB) and binary size delta between the shipped and bench builds are recorded, and the public-host cross-check is made; then each is checked or listed as open in the E-2 report.
...
- Given both binaries, when built for G4a, then `--version` on each carries the same commit and Cargo.lock hash (re-homed from E-8 in A-3b fix-pass 1; needs a build script, must land before G4a at the end of Sprint 4) and the harness asserts they match.
- Note (A-3b fix-pass 1, for E-2): add a redirect-chain memory check (5 hops, each with a body and a distinct client build) to the peak scenarios, since each hop builds a fresh client.
...
- **E-4 status (Sprint 5, PR #9).** The G4b scenarios (`g4b-window-start`, `g4b-window-end`, `g4b-raw`, `g4b-chunked-window-in-cap`, `g4b-window-beyond-cap`) are implemented (no longer refused as unimplemented) and run in the `bench-gate` peak step with G4a. G4b PASS on all four hosted cells (run 35641694726): gating peak 4.38 to 6.06 MiB, both 50 MiB chunked ratios at most 1.048. Not covered: the E-7-dependent A-4 checks.
- Re-homed from E-2 (pending owner acknowledgement, recorded Sprint 3 fix-pass 1): the `g6-concurrent10` scenario and the G4b scenario implementations are defined in the harness but marked not implemented (historic: implemented in Sprint 5, see the E-4 status for Sprint 5 and BENCHMARK s17); E-4 (with A-5 and A-6 for G4b) implements them. The E-2 macOS `/usr/bin/time -l` reader is not built (no macOS gate host); it is a known deviation from the E-2 acceptance text, also pending owner acknowledgement.
...
**E-6 status (Sprint 12, PR TBD).** Implemented inside the existing `bench-gate` job (`bench.yml`, no new job/matrix names, so no required-check-name change is needed once branch protection is configured): a new step runs `scripts/regression_gate.py`, comparing this run's idle median and gating-peak median against a stored per-cell `bench/baseline.json` and failing the job if either regressed more than 10% relative to that baseline — additive to, never a replacement for, the existing strict 10 MiB/40 MiB absolute-target gate. A new job, `update-baseline`, refreshes `bench/baseline.json` on every same-repo push to `main` (never on a PR), with `contents: write` scoped to that job only. Self-tested (`scripts/regression_gate_selftest.py`, run in `ci.yml`'s `test` job): PASS within threshold, FAIL beyond threshold (idle and peak independently), first-run-for-a-platform is advisory not a silent pass, and `--update` round-trips correctly. **Deviations from the AC text, disclosed:** (1) "the memory-gate job is added as a required status check" — the job cannot be added to branch-protection required checks by this agent (no repository-settings access, same constraint D-3 and D-7 already documented); the mechanism is implemented and the job id is unchanged (`bench-gate`), so the owner can add it (or its four matrix legs) when ready, exactly as already noted for the pre-existing absolute gate. (2) "the release workflow from D-2 is updated to depend on it" — NOT done: `release.yml`'s `publish` job is already hard-gated `if: false` (blocked on OQ-7, Sprint 9/D-2), so wiring a `needs:` dependency on `bench-gate` would be inert until that gate opens and is deferred to whoever does that OQ-7-driven edit, to avoid touching the already-reviewed `if: false` block for a dependency that cannot be exercised yet. (3) The regression-detection scope is idle + gating-peak only (the two figures the AC's "median idle or peak RSS" names), not every recorded/advisory scenario (`redirect-chain5`, `g6-concurrent10`, the hostile-HTML scenarios) — those stay recorded-only exactly as E-4 defined them.
...
- **A-5 status (Sprint 5, PR #9).** Implemented: streaming `Window` sink with early stop (connection closed once the window and one confirming character are seen), FR-02/FR-04 continuation, clamp, beyond-end and `max_length` 0 messages; chunked window inside the 5 MiB cap succeeds, beyond the cap gives `too_large`; G4b window scenarios are real `--gate` scenarios (BENCHMARK.md section 17). G4b PASS on all four hosted cells.
**Decisions needing owner acknowledgement (Sprint 5, recorded by the developer, not yet acknowledged):**
1. `[Total length: N characters.]` is stated only on a continuation page (`start_index` > 0) or in the beyond-end message, not on a first page that fits. This deviates from ADR-006 item 4; it keeps a small page "returned as fetched" (OQ-5 decision: no untrusted-content label).
2. Architecture G5 ("5 MiB body window beyond the cap" expecting `too_large`) cannot produce `too_large` with the 5 MiB fixture (it is under the cap). The 50 MiB chunked variant is used: `start_index` = the cap in characters (`g4b-window-beyond-cap`, and `g4a-50mib-chunked` now uses the same request).
3. G1 duplicates `g4a-5mib-full` (both are the 5 MiB page window at the end); `g4b-window-end` exists as a separate scenario so G4b can be run alone.
4. The `text/*` allowlist extras (`application/javascript`, `application/x-javascript`, `application/ecmascript`, `application/x-ndjson`, `+json`/`+xml` suffixes) are the developer's choice, beyond the AC list.
5. The Sprint 4 G4a read-in-full scenarios were redefined as window-at-end requests (100,000 characters), because early stop makes a start-0 read stop early.
...
- **A-6 status (Sprint 5, PR #9).** Implemented: content-type gating (text types and HTML converted unless `raw`; everything else, including `application/octet-stream`, gives `error[unsupported_content_type]` before any body byte is read) and untyped-body sniffing (BOM and whitespace tolerant); `g4b-raw` scenario and the idle re-check on the shipped binary (2.36 to 4.62 MiB, target 10); combined G4b record in BENCHMARK.md section 17. See the A-5 status for the decisions needing acknowledgement.
...
**B-6 status (Sprint 12).** Re-confirmed at Sprint 12 planning per the Sprints 6-12 table entry criterion ("OQ-5 decided"): the AC's third bullet ("Given OQ-5 is decided as 'no label', when the story is reviewed, then it is closed as won't-do") was already satisfied in Sprint 2 (Revision 10) and remains satisfied; no code change was made or needed. `src/server.rs`'s module doc and `tests/stdio.rs`'s `a5_...` test (both from Sprint 5) already state and assert the no-label behaviour. No further action.
...
- Given the binary, when inspected with `ldd`, then it links no OpenSSL and no runtime beyond libc. Note (A-3b fix-pass 1): reqwest's `rustls-no-provider` feature unavoidably compiles in the crates `rustls-platform-verifier` and `openssl-probe` (unused at runtime, no OpenSSL linkage, see the ADR-001 amendment); D-1 records their size but cannot trim them.
